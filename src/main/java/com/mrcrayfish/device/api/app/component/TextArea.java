...
public class TextArea extends Component
{
	...
	{
		super(left, top);
		this.fontRendererObj = Main.fontRendererObj;
		this.width = width;
		this.height = height;
	...
	public void render(Laptop laptop, Minecraft mc, int x, int y, int mouseX, int mouseY, boolean windowActive, float partialTicks) 
	{
		if (this.visible)
        {
			...
			if(!isFocused && placeholder != null && text.isEmpty())
			{
				...
				this.fontRendererObj.drawSplitString(placeholder, x + padding + 1, y + padding + 2, width - padding * 2 - 2, placeholderColour);
			}
	...
	public void writeText(String append)
	{
        ...
        if(fontRendererObj.listFormattedStringToWidth(newText + "_", width - padding * 2).size() <= maxLines)
		{
			this.text = newText;
		}
	}
